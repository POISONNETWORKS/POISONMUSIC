...
from POISONMUSIC.utils.inline import close_markup, stream_markup, stream_markup_timer
...
async def manage_callback(client, callback: CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await POISON.pause_stream(chat_id)
        ...
        await POISON.mute_stream(chat_id)
        ...
        await POISON.unmute_stream(chat_id)
...
async def handle_skip_replay(callback: CallbackQuery, _, chat_id: int, command: str, user_mention: str):
    ...
    if command == "Skip":
        ...
        try:
            ...
            if not playlist:
                ...
                return await POISON.stop_stream(chat_id)
        except Exception:
            try:
                await callback.edit_message_text(text_msg)
                await callback.message.reply_text(
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await POISON.skip_stream(chat_id, new_link, video=status, image=image)
        except Exception:
        ...
        try:
            ...
            image = None
        try:
            await POISON.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_6"])
        buttons = stream_markup(_, chat_id)
        img = await get_thumb(videoid)
        ...
        try:
            await POISON.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        buttons = stream_markup(_, chat_id)
        run = await callback.message.reply_photo(
        ...
        if videoid in ["telegram", "soundcloud"]:
            ...
            try:
                ...
                image = None
        try:
            await POISON.skip_stream(chat_id, queued, video=status, image=image)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        if videoid == "telegram":
            buttons = stream_markup(_, chat_id)
